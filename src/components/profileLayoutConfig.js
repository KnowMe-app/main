...
export const getProfileRole = user => {
  const roleValue = normalizeDisplayValue(user?.role || user?.userRole);
  const role = normalizeProfileRole(roleValue);
  if (role) return role;
  if (user?.__sourceCollection === 'newUsers' && !roleValue) return 'ed';
  return 'other';
};
