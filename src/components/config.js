...
export const readProfileFromNodes = async (userId, options = {}) => {
  ...
  // не почув.
  //
  // `readProfileNodePart` ковтає відмову навмисно — контакти можуть бути
  // закриті, і анкета мусить відкритись без них. Тому будь-яка розбіжність між
  // цим кодом і правилами бази виглядає назовні не як відмова, а як анкета, у
  // якої просто немає ані прізвища, ані контактів. Саме так і виглядала умова
  // «роль читача, відмінна від `ed`», яку правила колись вимагали додатково до
  // `feedDate`: донорка відкривала показану анкету агенції й бачила порожню
  // картку. Умову знято, обидві межі тепер питають одне й те саме — але
  // правила викочуються руками, тож розбіжність можлива знову (нерозгорнутий
  // `database.rules.json`), і причина має бути названа в консолі там, де її
  // видно.
