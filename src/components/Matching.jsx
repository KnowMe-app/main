...
const SwipeableCard = ({
  ...
  const quickFacts = getQuickFacts(user, resolvedRole);
  const sections = getProfileSections(user, resolvedRole);
  ...
  const triggerReaction = direction => {
    const wrap = direction === 'right' ? favoriteButtonWrapRef.current : dislikeButtonWrapRef.current;
  ...
  return (
    ...
    >
      <ModernProfileShell>
        <ModernSwipeHint>Swipe left to pass · right to like</ModernSwipeHint>
        ...
        {(contacts || isAdmin) && (
          <CardInfo>
            <HeaderIdentityRowSpaced>
              <Title $role={resolvedRole}>{roleLabel}</Title>
              {contacts && <IconsTrailing>{contacts}</IconsTrailing>}
              {isAdmin && <AdminToggle published={user.publish} onClick={e => { e.stopPropagation(); togglePublish(user); }} />}
            </HeaderIdentityRowSpaced>
          </CardInfo>
        )}
        <ModernProfileBody>
