...
export const normalizePhoneFieldValue = value => {
  if (Array.isArray(value)) {
    return value.map(item => normalizePhoneValue(item));
  }
...
export const normalizePhoneState = currentState => {
  if (!currentState || !Object.prototype.hasOwnProperty.call(currentState, 'phone')) {
    return currentState;
  }

  return {
    ...currentState,
    phone: normalizePhoneFieldValue(currentState.phone),
  };
};

  export const formatPhoneNumberNoSpace = number => normalizePhoneValue(number);
