...
const UI_TEXTS = {
  ...
  // А там, де відгуки приїжджають самі, плейсхолдер про читання мовчить.
  //
  // Відкрита картка, форма чернетки й форма доповнення питають `comments`
  // одразу, щойно відкрились, — тож кликати там «перевірити їх наявність»
  // означає просити зробити те, що вже зроблено, та ще й показувати стрілку
  // до значка, якого на цих екранах немає. Лишається сама робота: написати.
  // Про результат читання каже окремий рядок (`describeReviewsState`), а не
  // порожній плейсхолдер.
  publicCommentPlaceholderPlain: {
    en: 'Add a public note',
    uk: 'Додати публічну нотатку',
  },
