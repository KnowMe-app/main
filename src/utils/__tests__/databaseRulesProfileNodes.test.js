...
/**
 * Показ вирішує ключ стрічки — і більше ніщо.
 *
 ...
 * поруч із нею не лишилось іншої гілки: ані ролі читача, ані чогось, що
 * відкриє анкету без `feedDate`.
 *
 * Роль читача тут колись була — умова «роль, відмінна від `ed`». Знята вона
 * навмисно: код, який складає анкету (`profileVisibilityScope.js`), її не
 * повторював, тож донорка бачила показану анкету агенції без прізвища й
 * контактів — правила відмовляли, а відмову `readProfileNodePart` ковтав.
 * Тепер обидві межі кажуть одне: є дата в `feedDate` — анкета читається, немає
 * — лишається сама картка.
 */
const expectFeedGatedRead = read => {
  ...
  // Роль не питається ніде: ані до ключа стрічки, ані після нього.
  expect(read).not.toContain('userRole');
  expect(read).not.toContain("child('role')");
  const [beforeGate] = read.split(FEED_GATE);
...
describe('matchingCards приймає лише перелічені поля', () => {
  ...
  it('дозволяє власнику прочитати власну картку', () => {
    // Саме `feedDate` каже, чи анкета показана, тож без права на власну картку
    // звичайний користувач не дізнався б власного стану публікації. І саме тому
    // власник названий окремо: сховану картку (`feedDate: false`) правило не
    // віддає нікому, крім нього, адмінів і службового доступу.
    expect(card['.read']).toContain('auth != null');
...
describe('profileContacts — власне правило, і воно йде за стрічкою', () => {
  ...
  it('звичайному користувачеві — тільки поки картка в стрічці', () => {
    /*
     * Контакти показаної анкети читає кожен авторизований — тим показ і є.
     * Межа стоїть на самій анкеті, а не на тому, хто її читає: є `feedDate` —
     * контакти віддаються; немає — максимум, що лишається читачеві, це проєкція
     * `matchingCards`.
     */
    expectFeedGatedRead(rules.profileContacts.$uid['.read']);
  });
...
describe('решта вузлів не стає другим місцем для чужих даних', () => {
  ...
  it('profileDetails звичайному користувачеві — теж тільки поки картка в стрічці', () => {
    ...
    expectFeedGatedRead(rules.profileDetails.$uid['.read']);
  });
