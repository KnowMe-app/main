...
describe('Contacts', () => {
  ...
  it('зливає два списки замість того, щоб оголосити їх конфліктом', () => {
    // Анкета тримає кілька телефонів, і дві колекції — це дві частини одного
    // списку, а не дві відповіді. Раніше тут був конфлікт, і поле не переїжджало
    // взагалі: з двох частин виходив нуль.
    const state = stateWith({ P1: { phone: ['+380'] } }, { P1: { phone: ['+381'] } });
    const plan = runMigrationGroup(state, 'profileContacts');

    // Порядок той самий, що його показував застосунок: значення з `users` —
    // останнє, бо саме воно всюди читається як поточне.
    expect(state.targets.profileContacts.P1.phone).toEqual(['+381', '+380']);
    expect(plan.conflicts).toEqual([]);
    expect(plan.warningsByCode.LIST_VALUES_MERGED).toBe(1);
    // Обидва джерела віддали своє — тримати їх далі нема за чим.
    expect(state.workingNewUsers.P1).toEqual({});
  });

  it('зливає список і скаляр, а не втрачає одного з них', () => {
    const state = stateWith({ P1: { phone: '+380' } }, { P1: { phone: ['+381', '+382'] } });
    runMigrationGroup(state, 'profileContacts');

    expect(state.targets.profileContacts.P1.phone).toEqual(['+381', '+382', '+380']);
  });
