...
/**
 * Кладе анкету в кеш і повертає **збережене**, а не передане.
 *
 * Різниця не косметична: `sanitizeMatchingCardForCache` навмисно не пускає в
 * кеш контакти того читача, чиє право на них тримається на `feedDate`, і важкі
 * поля на кшталт `photos`. Тобто повернене тут завжди бідніше за `data` — і
 * показувати його замість `data` означає загубити те, що вже прочитали з бази.
 * Саме так контакти агенції зникали дорогою до екрана донорки
 * (`fetchUsersByIds`). Кому потрібна анкета для показу — бере `data`, а не
 * результат цього виклику; результат тут для тих, кому треба знати, що саме
 * лягло в сховище.
 */
