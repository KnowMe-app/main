...
не повторює. Роль береться з `matchingCards/{uid}/role` (це те, що пише застосунок, коли
людина міняє роль на `MyProfile`), а legacy `users/{uid}/userRole` лишається відповіддю
лише для акаунтів, чия картка ролі ще не має або тримає їх кілька. Тобто відмова можлива на анкеті, яку код вважає відкритою, а
...
- Стрічка не показує донорці чужих донорських анкет (`hidePeerDonorCards`), але пошук їх
  знаходить: у стрічці питання «кого мені шукати», у пошуку — «де конкретна людина».
  Правило діє лише в деці за замовчуванням; власна анкета й окремо надані картки з нього
  виключені.
